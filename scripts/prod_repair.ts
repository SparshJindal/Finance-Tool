...
async function main() {
  ...
  if (!user) {
    console.error(`User with email ${userEmail} not found in the connected database.`)
    console.error(`Ensure DATABASE_URL points to the production database containing your 7 holdings.`)
    return
  }
  ...
  // 3. Re-ingest (skipHeavyApis = false, so Earnings Pass fires)
  console.log(`\nStarting Re-ingest for all holdings with heavy APIs (skipHeavyApis = false)...`)
